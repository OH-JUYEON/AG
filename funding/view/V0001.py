...
from region.models import CityDetail, CityHeader
import json
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if "전체" in data['city_detail_name']:
                city = data['city_detail_name'].replace( "전체","").strip()  
                city_id = CityHeader.objects.filter(Q(city_name__icontains=city))[0].id
        
                if data['type'] == 1:
                    # 펀딩 최신순
                    if data['catetype'] == 0:
                        dataList = Funding.objects.filter(status=1, campaign__city_header_id=city_id).order_by('-id')
                    # 펀딩 참여순
                    elif data['catetype'] == 1:
                        dataList = Funding.objects.annotate(funding_sponsor_count=Count('fundingsponsor__funding_id')).filter(status=1,campaign__city_header_id=city_id).order_by('-funding_sponsor_count')

                elif data['type'] == 2:
                    # 기부 최신순
                    if data['catetype'] == 0:
                        dataList = Donation.objects.annotate(donation_doner_count=Count('donationdoner__donation_id')).filter(status=1,city_header_id=city_id).order_by('-id')
                    # 기부 참여순
                    elif data['catetype'] == 1:
                        dataList = Donation.objects.annotate(donation_doner_count=Count('donationdoner__donation_id')).filter(status=1,city_header_id=city_id).order_by('-donation_doner_count')
                ...
                if data['type'] == 1:
                    ...
                    # 기부 최신순
                    if data['catetype'] == 0:
                        dataList = Donation.objects.annotate(donation_doner_count=Count('donationdoner__donation_id')).filter(city_detail_id=city[0].id,status=1).order_by('-id')
                    # 기부 참여순
                    elif data['catetype'] == 1:
                        dataList = Donation.objects.annotate(donation_doner_count=Count('donationdoner__donation_id')).filter(city_detail_id=city[0].id,status=1).order_by('-donation_doner_count')
