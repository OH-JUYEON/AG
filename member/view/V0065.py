...
class ReviewList(View):
    ...
    def get(self, request, *args, **kwargs):     
        ...
        try:
            ...
            reviewList = CampaignReview.objects.filter(member_id=member_id,campaign_review_status=1).order_by('-id')[start:end]
            dataList = {}
            ...
            # return render(request, 'mypage/mypage__007/_T007.html', {'dataList':dataList})
         
        except json.JSONDecodeError as e:
            return JsonResponse({'error': 'Invalid JSON'}, status=400)

        return JsonResponse({'result': json.dumps(dataList)}, status=200)
